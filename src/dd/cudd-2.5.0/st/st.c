...
#define EQUAL(func, x, y) \
    ((((func) == st_numcmp) || ((func) == st_ptrcmp)) ? (ST_NUMCMP((x), (y)) == 0) : ((*func)((x), (y)) == 0))

#define do_hash(key, table)                                                                                                                                         \
    ((int) ((table->hash == st_ptrhash) ? ST_PTRHASH((char*) (key), (table)->num_bins) : (table->hash == st_numhash) ? ST_NUMHASH((char*) (key), (table)->num_bins) \
