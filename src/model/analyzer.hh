...
typedef vector<ExprKind> KindStack;
...
class BadExpressionKindException : public Exception {
public:
    BadExpressionKindException(ExprKind encountered, Expr_ptr expr, ExprKinds allowed)
        : f_encountered(encountered)
        , f_allowed(allowed)
        , f_expr(expr)
    {}

    ~BadExpressionKindException() throw()
    {}

    virtual const char* what() const throw() {
        ostringstream oss;
        oss << "Unsupported expression kind detected in "
            << f_expr
            << " (found:" << f_encountered
            << ", allowed: " << f_allowed << ")" << endl;

        return oss.str().c_str();
    }

private:
    ExprKind f_encountered;
    ExprKinds f_allowed;
    Expr_ptr f_expr;
};
...
private:
    ...
    inline bool cache_miss(const Expr_ptr expr)
    {
        ...
        KindMap::iterator eye = f_map.find(key);

        if (eye != f_map.end()) {
            ExprKind res = (*eye).second;
    ...
    // post-orders only
    void walk_ltl_g(const Expr_ptr expr);
    void walk_ltl_unary(const Expr_ptr expr);
    void walk_ltl_binary(const Expr_ptr expr);

    void walk_ctl_ag(const Expr_ptr expr);
    void walk_ctl_unary(const Expr_ptr expr);
    void walk_ctl_binary(const Expr_ptr expr);

    void walk_unary_boolean(const Expr_ptr expr);
    void walk_unary_algebraic(const Expr_ptr expr);

    void walk_binary_algebraic(const Expr_ptr expr);
    void walk_binary_relational(const Expr_ptr expr);
    void walk_binary_boolean(const Expr_ptr expr);

    void walk_ternary_ite(const Expr_ptr expr);

    // low-level checkers
    ExprKinds f_temporal;
    void check_temporal(Expr_ptr expr, ExprKind ek);

    ExprKinds f_ltl;
    void check_ltl(Expr_ptr expr, ExprKind ek);

    ExprKinds f_ctl;
    void check_ctl(Expr_ptr expr, ExprKind ek);

    ExprKinds f_boolean;
    void check_boolean(Expr_ptr expr, ExprKind ek);

    ExprKinds f_algebraic;
    void check_algebraic(Expr_ptr expr, ExprKind ek);

    // used by ite
    void check_match(Expr_ptr expr, ExprKind lhs, ExprKind rhs);
};
