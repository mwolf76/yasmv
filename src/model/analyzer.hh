...
// ctx-expr-type entry
typedef stack<Type_ptr> TypeStack;

class Analyzer : public Walker {
private:
  TypeMap f_map;
  TypeStack f_stack;
  ...
  // for fqdn names resolution
  unsigned f_id_depth;
  Exprs f_id_fragments;
...
protected:
  ...
  inline Type_ptr lookup(const Expr_ptr expr)
  {
    TypeMap::const_iterator eye = f_map.find(FQExpr(f_ctx, expr));
    return ( (eye != f_map.end()) ? (*eye).second : NULL );
  }

  ISymbol_ptr resolve(const Expr_ptr ctx, const Expr_ptr id);

  // type inference
  Type_ptr infer_expr_type(Expr_ptr ctx, Expr_ptr body);
  Type_ptr infer_expr_type(Expr_ptr body);
