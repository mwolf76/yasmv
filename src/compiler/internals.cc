...
namespace compiler {
    ...
    ADD Compiler::make_auto_dd()
    {
        ...
        enc::BooleanEncoding_ptr be(reinterpret_cast<enc::BooleanEncoding_ptr>(f_enc.make_encoding(boolean)));
    ...
    void Compiler::pre_node_hook(expr::Expr_ptr expr)
    {
        ...
        expr::TimedExpr key(f_owner.em().make_dot(ctx, expr), time);
       ...
       anonymous determinization variables on the fly. */
        ...
        if (type->is_scalar()) {
            ...
            if (width > f_add_stack.size()) {
                unsigned add_stack_size(f_add_stack.size());
            ...
            expr::Expr_ptr timedExpression(em.make_at(em.make_instant(time), em.make_dot(ctx, expr)));

            /* memoize result */
            f_compilation_cache.insert(std::pair<expr::TimedExpr, Unit>(key, Unit(timedExpression, dv, f_inlined_operator_descriptors,
    ...
    bool Compiler::cache_miss(const expr::Expr_ptr expr)
    {
        ...
        expr::TimedExpr key(f_owner.em().make_dot(f_ctx_stack.back(), expr), f_time_stack.back());
        ...
        if (eye != f_compilation_cache.end()) {
            ...
            {
                const InlinedOperatorDescriptors& vec(unit.inlined_operator_descriptors());
            ...
            {
                const Expr2BinarySelectionDescriptorsMap& map(unit.binary_selection_descriptors_map());
            ...
            {
                const MultiwaySelectionDescriptors& vec(unit.array_mux_descriptors());
