...
namespace axter
{
  void display_stack_main(int);

  /*! @struct levels
    @brief levels is a struct used to hold extended data for the format policy.
    Example Usage

    @verbatim EZLOGGERVLSTREAM(axter::levels(axter::log_often,
    axter::warn, "Xyz Facility")) << somedata << " " << i <<
    std::endl;
  ...
  */
  ...
  class ezlogger : public EZLOGGER_OUTPUT_POLICY,
                   public EZLOGGER_FORMAT_POLICY,
                   public EZLOGGER_VERBOSITY_LEVEL_POLICY
  {
  public:
    ...
    bool log_if_fails_verification(bool eval, const char* evaluation)
    {
      if (!eval && m_verbosity_level <= get_verbosity_level_tolerance())
        get_log_stream() <<
          get_log_prefix_format(m_src_file_name,
                                m_src_line_num,
                                m_src_function_name,
                                m_levels_format_usage)
                         << "Failed verification:  '"
                         << evaluation << "'" << std::endl;
      return eval;
    }
